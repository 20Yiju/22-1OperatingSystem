...
int main(int argc, char *argv[])
{
	...
	if(argc > 1){
		...
		no_tickets = atoi(argv[2]);
	}
	else{
		printf("Usage: ./hw7_2 <no_thread> < no_tickests>\n");
		return -1;
	}
	...
	if(sem_init(&sem, 0, no_tickets) != 0){
		printf("sem init failed\n");
		return -1;
	}
	sem_getvalue(&sem, &sema_val);
...
void* ThreadFn(void *vParam)
{
	...
	while(thread_cont){
		...
		if(oldy == param -> critical_bottom + 1){
			sem_post(&sem);
			sem_getvalue(&sem, &sema_val);
		}
		oldy = y;	
	...
	while(sema_val < param->no_tickets){
		sem_post(&sem);
		sem_getvalue(&sem, &sema_val);
	}
	pthread_exit(0);
